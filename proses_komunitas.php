...
// 1. Proteksi Akses: Wajib login
if (!isset($_SESSION['username'])) {
    header("Location: login.php");
    exit();
}

// 2. Proteksi Akses: Validasi CSRF Token dari request POST (AJAX FormData)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($file_target !== null) {
        ...
        if ($jenis_media === 'gambar') {
            ...
            $pesan_ukuran_error = 'Ukuran video terlalu besar (Maksimal 100MB).';
        ...
        if (in_array($ekstensi_file, $ekstensi_diperbolehkan) || $jenis_media === 'video') { 
            if ($file_size <= $ukuran_maksimal) {
                ...
                // Memastikan penentuan ekstensi akhir aman
                $ekstensi_final = ($jenis_media === 'video') ? (in_array($ekstensi_file, ['mp4', 'webm', 'mov', 'mkv', '3gp']) ? $ekstensi_file : 'mp4') : $ekstensi_file;
